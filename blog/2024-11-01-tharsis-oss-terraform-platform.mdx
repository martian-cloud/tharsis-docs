...
export function DemoVideo() {
    return <ReactPlayer sx={{ borderRadius: 4 }} width="100%" height="100%" playing muted controls url={useBaseUrl('posts/tharsis-oss-terraform-platform/demo.mp4')} />
}

We are excited to announce a new open-source Terraform platform called Tharsis! Tharsis is a free open-source project which offers an alternative to using the HashiCorp Terraform Cloud Platform (HCP). 
...
authentication mechanism. This allows CICD runners such as GitHub Actions or GitLab Jobs to assume a service account using a JWT token that is compatible with the OIDC protocol.
...
to the VCS provider. This allows you to automate your Terraform deployments and keep your infrastructure in sync with your codebase without having to manage CICD runners and pipelines.
