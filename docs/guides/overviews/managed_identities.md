...
Yes, you can create multiple AWS managed identities within a group and assign them to a workspace. However, non-AWS managed identity types are restricted to one per workspace. For more details, refer to [Assign multiple managed identities](#assign-multiple-managed-identities).
